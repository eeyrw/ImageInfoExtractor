...
class SmartCrop(object):  # pylint:disable=too-many-instance-attributes
    ...
    def crop(  # pylint:disable=too-many-arguments,too-many-locals
        ...
        if prescale:
            ...
            if prescale_size < 1:
                image = image.copy()
                image.thumbnail(
                    (int(math.ceil(image.size[0] * prescale_size)), 
                     int(math.ceil(image.size[1] * prescale_size))),
                    Image.Resampling.LANCZOS)
                crop_width = int(math.floor(width/min_scale))
                crop_height = int(math.floor(height/min_scale))
    ...
    def crops(  # pylint:disable=too-many-arguments
        ...
        for scale in [i/100 for i in scale_list]:
...
class SmartCropWithFace(SmartCrop):
    def __init__(self, 
                 faceDetWeight:str = 'DLToolWeights/SmartCrop/face_detection_yunet_2023mar.onnx',
                 detail_weight: float = 0.2, edge_radius: float = 0.4, edge_weight: float = -20,
        ...
        self.faceDtr = FaceDetector(modelPath=faceDetWeight)
...
if __name__ == '__main__':
    ...
    cropper = SmartCropWithFace(debug=False)

    ret = cropper.crop(img.copy(), 80, 80,max_scale=1.0,min_scale=0.9)
